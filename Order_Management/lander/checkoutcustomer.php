...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("
        INSERT INTO customers (name, email, phone_no, order_date, order_time, created_at)
        VALUES (?, ?, ?, ?, ?, NOW())
    ");
    $stmt->bind_param("sssss", $name, $email, $phone_no, $order_date, $order_time);
        ...
        <div class="col-lg-5">
            <div class="checkout-right order-summary">
                ...
                <?php else: ?>
                    ...
                    <?php if($discount>0): ?>
                        <div class="alert alert-success">🎉 Discount: $<?= number_format($discount,2) ?> | Total: $<?= number_format($discountedTotal,2) ?></div>
                    <?php else: ?>
